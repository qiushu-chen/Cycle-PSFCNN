...
try:
    from pytorch_grad_cam import (EigenCAM, EigenGradCAM, GradCAM,
                                  GradCAMPlusPlus, LayerCAM, XGradCAM, AblationCAM, 
                                  AblationLayer, ActivationsAndGradients)
...
class MMActivationsAndGradients(ActivationsAndGradients):
    ...
    def __call__(self, x):
        self.gradients = []
        self.activations = []
        return self.model(x, return_loss=False, sigmoid=False, post_process=False)
        # return self.model(
...
def main():
    ...
    if args.method != "ablationcam":
        ...
        batch_size = ablationcam_extra_params.get('batch_size', 1)
        ratio_channels_to_ablate = ablationcam_extra_params.get(
                'ratio_channels_to_ablate', 1.)
        cam = AblationCAM(model, target_layers, use_cuda, reshape_transform,
                          batch_size=batch_size,
                          ablation_layer=ablationcam_extra_params['ablation_layer'],
    ...
    tifdatadirs = os.listdir(args.img)
    for tifdir in tifdatadirs:
        tifdirnew = tifdir.replace('.tif', '_a.jpg')
        save_path_err = str(args.save_path)
        save_path_err = save_path_err.replace('\\','/')
        #print(save_path_err)
        out_patha = save_path_err + '/' + tifdirnew
        tifdir_path = args.img + '/' + tifdir
        data, src_img = apply_transforms(tifdir_path, cfg.data.test.pipeline)
        grayscale_cam = cam(
        ...
        show_cam_grad(
            grayscale_cam, src_img, title=args.method, out_path=out_patha)
