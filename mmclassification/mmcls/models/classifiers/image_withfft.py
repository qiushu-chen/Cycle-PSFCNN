...
class ImageFFTClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 init_cfg=None):
        super(ImageFFTClassifier, self).__init__(init_cfg)
    ...
    def forward_train(self, img, gt_label, **kwargs):
        ...
        #     fft_temp = torch.fft.fftshift(torch.fft.fft2(img[i,1,:,:], dim=(-2,-1)))
        #     fft_temp = torch.stack((fft_temp.real, fft_temp.imag), 0)
        #     img_fft_tensor[i,:,:,:] = fft_temp
        if self.augments is not None:
            img, gt_label = self.augments(img, gt_label)
        # img_cat_fft = torch.cat((img,img_fft_tensor),1)
        # transform_g = transforms.Grayscale()
        # img_g = transform_g(img)
        # x = self.extract_feat(img_cat_fft)
        x = self.extract_feat(img)

        losses = dict()
        cls_score, loss = self.head.forward_train(x, gt_label)
    ...
    def simple_test(self, img, img_metas=None, **kwargs):
        ...
        # img0 = img.cuda()
        # for i in range(img_size[0]):
        #     fft_temp = torch.fft.fft2(img[i,1,:,:], dim=(-2,-1))
        #     fft_temp = torch.stack((fft_temp.real, fft_temp.imag), 0)
        #     img_fft_tensor[i,:,:,:] = fft_temp
        # print(img.device, img_fft_tensor.device)
        # img_fft_tensor0 = img_fft_tensor.to('cpu')
        # img_cat_fft = torch.cat((img,img_fft_tensor),1)
        # x = self.extract_feat(img_cat_fft)
        x = self.extract_feat(img)
