...
def get_wavefront_torch(x,y,params,cnn_pa):
    ...
    aberration = cnn_pa[4]*torch.pow(r2,2) + cnn_pa[1]*r2 + cnn_pa[2]*torch.pow((x*torch.cos(cnn_pa[3])+ y*torch.sin(cnn_pa[3])),2) + \
            params.coma * ((x*r2)*torch.cos(torch.tensor([params.coma_angle])) + (y*r2)*torch.sin(torch.tensor([params.coma_angle]))) + \
            cnn_pa[5]*(x*torch.cos(cnn_pa[6])+ y*torch.sin(cnn_pa[6]))
    wavefront = torch.exp(2*1j*np.pi*aberration)
    # print(wavefront)
    return wavefront

def get_psf(params, cnn_pa, centered = True):
    datapoints = np.int(params.size)
    # print(datapoints)
...
def rl_deconvolution_torch(img_list, psf_list, iterations=40, lbd=0.2):
    ...
    for k in range(len(img_list)):
        ...
        latent_final_estimate1 =  max_value_temp[k] - 1.5*latent_final_estimate1
...
path = "../../../fudan2024_1/202403/lxa"
path1 = "../../../fudan2024_1/202403/lxa_lqsy"
...
f1 = open("F://fudan2024_1/202403/lxa/glass_test.txt",'a')
for tifdir in tifdatadirs:
    if os.path.isdir(path + '/' + tifdir):
        ...
        for tiffile in tifdatanames:
            ...
            if (img_ratio > 0.0003 and img_var > 2.4e-08):
                para_valid = 0
            else:
                para_valid = 1
            ...
            for img_idx in range(5):
                para_ast = 0.5*np.random.rand()
                para_ast_angle = np.pi*np.random.rand() - 0.5*np.pi
            # para_ast = 0.0
            # para_ast_angle = 0.0
            # para_temp = np.random.rand()
                if para_ast>0.3:
                    para_focus = 1.0*np.random.rand() - 0.5
                else:
                    para_focus = 4.0*np.random.rand() - 2.0
                para_sph = 0.5*np.random.rand()
                para_tilt = 0.
                para_tilt_angle = 0.
            
                psf_array = torch.tensor([para_valid, para_focus, para_ast, para_ast_angle, para_sph, para_tilt, para_tilt_angle]).requires_grad_(True)
            ...
            # psf_list = psf_list.requires_grad_(True)
                img_out_list = conv_with_torch(img_list, psf_list)
            ...
            # print(psf_array) 
                ...
                f1.write(tifdir + '/' + tiffilenew + ' '+ str(para_valid) + ',' + str(para_focus)+','+str(para_ast)+','+str(para_ast_angle)+','+str(para_sph)+'\n')
